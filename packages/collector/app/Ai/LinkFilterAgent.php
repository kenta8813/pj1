<?php

namespace App\Ai;

use Laravel\Ai\Attributes\MaxTokens;
use Laravel\Ai\Attributes\Provider;
use Laravel\Ai\Attributes\Temperature;
use Laravel\Ai\Contracts\Agent;
use Laravel\Ai\Enums\Lab;
use Laravel\Ai\Promptable;

#[Provider(Lab::OpenRouter)]
#[Temperature(0.0)]
#[MaxTokens(512)]
class LinkFilterAgent implements Agent
{
    use Promptable;

    public function instructions(): string
    {
        return <<<'PROMPT'
        あなたは日本の自治体ウェブサイトのナビゲーターです。
        与えられたプロンプトの指示に従い、関連するURLのみをJSON配列で返してください。

        子育て支援の関連カテゴリ（広めに判断すること）：
        保育園・幼稚園・こども園・認定こども園、手当・補助金・給付金・助成、
        医療費助成・乳幼児医療、母子手帳・妊婦健診・産前産後ケア、
        育児相談・子育て支援センター、学童保育・放課後・ファミリーサポート、
        障害児支援・発達相談、入学・就学援助・教育支援

        給付金・助成金の関連カテゴリ（広めに判断すること）：
        児童手当・児童扶養手当・特別児童扶養手当、乳幼児医療費助成・子ども医療費助成、
        保育料助成・無償化、出産育児一時金・出産応援給付金、おむつ応援・育児用品助成、
        就学援助・奨学金、ひとり親家庭等医療費助成、障害児通所給付

        返答形式：関連URLのみをJSON配列で返す（説明・コードブロック不要）
        例: ["https://...", "https://..."]
        関連URLがなければ空配列 [] を返す。
        PROMPT;
    }
}
